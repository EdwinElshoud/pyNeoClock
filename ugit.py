...
user = 'EdwinElshoud'
repository = 'pyNeoClock'
token = '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'
...
ignore_files = ['/README.md', '/ugit.py', '/boot.py']
ignore = ignore_files
...
tags_url = f'https://api.github.com/repos/{user}/{repository}/commits?per_page=1'
tag_file = 'ugit.tag'

def find_new_version(headers=None):
    print(tags_url)
    if headers is None:
        headers = get_headers()
    ...
    response = urequests.get(tags_url, headers=headers)
    ...
    try:
        ...
        print(f'No {tag_file} existing yet...')
        
    if tag_local is None or tag_local != tag_git:
...
def pull(f_path,raw_url, headers=None):
...
def pull_all(tree=call_trees_url,raw = raw,ignore = ignore,isconnected=False):
  ...
  headers = get_headers()
  new_tag = find_new_version(headers)
  if new_tag is None:
  ...
  for i in tree['tree']:
    if i['type'] == 'tree':
      ...
      try:
        pull(i['path'],raw + i['path'], headers=headers)
...
def pull_git_tree(tree_url=call_trees_url,raw = raw):
  headers = {'User-Agent': 'ugit-ee-upy'} 
  # ^^^ Github Requires user-agent header otherwise 403
  if len(token) > 0:
      headers['authorization'] = "bearer %s" % token
  print(tree_url)
